...
pub struct SAAuthResponsePacket {
    pub success: bool,
}

impl SAAuthResponsePacket {
    pub fn parse(packet: Packet) -> Option<Self> {
        if packet.id != ID::SAAuthResponse {
            return None;
        }
    ...
    pub fn to_string(&self) -> Result<String, serde_json::Error> {
        ...
        let packet = Packet::new(Version::V0_1_0, ID::SAAuthResponse, data);
        serde_json::to_string(&packet)
    }

    pub fn to_packet(&self) -> Packet {
        let data = serde_json::to_value(&self).expect("packet data should be serializeable");
        Packet::new(Version::V0_1_0, ID::SAAuthResponse, data)
    }
}
